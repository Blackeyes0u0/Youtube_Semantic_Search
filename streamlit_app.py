...
delicious food with meet"""
    ...
    like_instruction = st.sidebar.text_area("Enter a YouTube video like instruction", value="i want to see cute animals and", help="please enter what you want to see video")
    hate_instruction = st.sidebar.text_area("Enter a YouTube video hate instruction", value="bird", help="please enter what you don't want video")
...
# 캐싱된 검색 함수
@st.cache_data
def retrieval(texts,top_rank,API_KEY):
    query_vectors = query2emb(texts)
    result = {}
...
def main_ui():
    ...
    if 'texts' in st.session_state and 'top_rank' in st.session_state:
        result = retrieval(st.session_state.texts, st.session_state.top_rank,st.session_state.API_KEY)
